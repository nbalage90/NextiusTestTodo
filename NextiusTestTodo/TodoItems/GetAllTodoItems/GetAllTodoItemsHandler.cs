...
public record GetAllTodoItemsQuery(int? PageSize, int PageNumber = 1, bool? StatusFilter = null) : IRequest<GetAllTodoItemsResult>;
...
public class GetAllTodoItemsHandler(ITodoItemRepository repository) : IRequestHandler<GetAllTodoItemsQuery, GetAllTodoItemsResult>
{
    ...
    {
        var todoItemEntities = await repository.GetAllAsync(cancellationToken, request.PageSize, request.PageNumber, request.StatusFilter);
